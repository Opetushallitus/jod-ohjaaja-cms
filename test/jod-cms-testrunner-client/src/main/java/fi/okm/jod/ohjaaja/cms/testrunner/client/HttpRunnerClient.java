...
public final class HttpRunnerClient {
  ...
  public long installBundle(byte[] bundleBytes) throws IOException {
    ...
    if ((conn.getResponseCode() / 100) != 2) {
      throw new IOException("Bundle install failed: HTTP " + conn.getResponseCode() + " - " + body);
    }
  ...
  public void uninstallBundle(long bundleId) throws IOException {
    ...
    if ((code / 100) != 2) {
      throw new IOException("Bundle uninstall failed: HTTP " + code + " - " + body);
    }
  }
  ...
  private static String buildRunPayload(
      long bundleId, String className, List<String> excludedMethods) {
